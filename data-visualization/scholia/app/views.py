...
def __breadth_first_search(query_keyword, current_hop, max_hops, already_visited, result, keywords, authors, max_n=3):
    ...
    if query_keyword not in already_visited and current_hop <= max_hops:
        ...
        for author in authors_in_keyword:
            ...
            _new_entry = {
                "author1": {
                    "type": "uri",
                    "value": f"http://localhost:8100/keyword?kw={encoded}&dist=1&n={max_n}"
                },
                "author2": {
                    "type": "uri",
                    "value": f"http://localhost:8100/author?name={_encoded_name}&dist=1&n={max_n}"
                },
                "author1Label": {
                    "xml:lang": "en",
                    "type": "literal",
            ...
            if _name in authors:
                ...
                for related_keyword in other_keywords:
                    ...
                    _new_entry = {
                        ...
                        "author2": {
                            "type": "uri",
                            "value": f"http://localhost:8100/keyword?kw={related_encoded}&dist=1&n={max_n}"
                        },
                        "author1Label": {
                            "xml:lang": "en",
                            "type": "literal",
                    ...
                    if related_keyword['topicLabel'] not in rgb_map:
                        rgb_map[related_keyword['topicLabel']] = d3_category20c[rgb_idx]
                        rgb_idx += 1
                        if rgb_idx == len(d3_category20c):
                            rgb_idx = 0
...
def index():
    ...
    return render_template('cluster.html',
                           ...
                           last_name=last_name,
                           cluster_keywords=json.dumps(__get_keywords_bubble_chart_data())
                           )
